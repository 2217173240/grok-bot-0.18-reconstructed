...
添加本地 MCP 服务时，在 `~/.grokbot-local/plugins/mcp-servers.json` 中写入标准的 `mcpServers` 对象。旧位置 `~/.grokbot-local/mcp-servers.json` 的文件会在下次启动时移入该目录。[扩展说明](docs/EXTENSIBILITY.md)列出 provider、工具与插件的接入位置。
