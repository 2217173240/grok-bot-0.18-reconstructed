...
export function computerUseSubagentDescription(browserUseOffered: boolean): string {
  return [
    ...(browserUseOffered ? [
      "Delegate a self-contained desktop task to a subagent that drives your box's desktop — GUI apps, file dialogs, drag interactions, and sites that defeat page-level automation — by screenshot, click, drag, type, key, scroll, and wait.",
      ...
      "Delegate a self-contained GUI / desktop task to a subagent that drives your box's desktop — browsing, signing in to sites, and using GUI apps — by screenshot, click, drag, type, key, scroll, and wait.",
    ]),
    displaySpaceSentence(),
    "Task waits for its result by default. With run_in_background=true, finish your current turn after dispatch; its completion arrives in a later turn. Do not use AwaitShell to wait for subagents.",
