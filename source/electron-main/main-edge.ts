...
import { COMMAND_CODE_DEFAULT_MODEL, isCommandCodeModelId, isSandInferenceProvider } from "../shared/inference-router.js";
...
export function createMainEdgeHandlers(deps: MainEdgeDeps): HandlerMap {
  const handlers: HandlerMap = {
    ...
    getInferenceRouter: async () => { const settings = await deps.readHostSettingsFromBox().catch(() => ({} as UnknownRecord)); const chosen = (invoke(deps.settingsStore, "load") as UnknownRecord).inferenceProvider; const provider = isSandInferenceProvider(chosen) ? chosen : isSandInferenceProvider(settings.inferenceProvider) ? settings.inferenceProvider : invoke(deps.settingsStore, "getInferenceProvider"); return { provider: isSandInferenceProvider(provider) ? provider : "cursor", usage: settings.inferenceRouterUsage ?? invoke(deps.settingsStore, "getInferenceRouterUsage") ?? null, local: getLocalInferenceCliStatus() }; },
