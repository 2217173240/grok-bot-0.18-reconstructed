...
**可选迁移（从源机拷）**：`plugins/mcp-servers.json` + 对应插件的服务器文件（本地 MCP 插件源，服务器文件的路径按共享工作区写，
例如 `/workspace/demo-mcp-server.cjs`）、`box-secrets.json`（Saved keys 镜像）。不拷则插件面为空，不影响主线。

`mcp-servers.json` 放在 Mac 数据根的 `plugins/` 目录中，该目录总是以只读方式绑定到 `/home/box/sand-data/plugins`，即盒内定义源
读取的位置。绑定目录而不是单个文件，因此以替换方式保存的改动和容器创建后才新建的文件，盒子都能读到当前内容；
旧位置（数据根下的 `mcp-servers.json`）的文件会在下次启动时移入 `plugins/`。盒子里的 stdio 插件服务器由盒内
