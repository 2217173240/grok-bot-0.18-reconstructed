...
未修但已登记：Mac 平面的路由器丢弃附件/回复/fork/提及（影响远程盒子部署，本部署不走该路径）、
盒内无插件 MCP 工具、Codex 凭据在盒内不可达、权限设置对盒内 CLI 工具无效（本部署由容器边界兜住）、
转录条目 id 按平面各自生成、接管 URL 无新鲜度检查、`stop` 未核实、门禁 G4 把 exec 失败强制成 0 后放行。
