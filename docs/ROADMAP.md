...
**归档文档的一轮外部复核（2026-09-23）**：复核逐条对照源码后提出八处问题，全部核实成立并已修正两份文件。要点：
把"AI 引擎只在远端盒里"改为"host 有三种运行位置，且回合调度平面与模型服务位置要分开看"（`resolveLocalAdminBox`
可返回 `mac-host`，`SAND_LOCAL_ADMIN_TURN=mac` 时路由器在 Mac 侧执行）；把"所有输出统一走 `InteractionUpdate`"改为
"两条事件产生路径"（路由器直接合成 transcript 条目并 `postEvent`）；把"步数上限保证收敛"改为"步数预算 + 特定故障
处理"（上限约束循环次数，超时、取消与任务完成各自另有保证）；把"隐私默认剥离"改为"策略可判定，是否执行由
`isGlobalEnforcementEnabled()` 决定，其默认值为 `false`"；把交付检查分为输入身份、产物组成、启动检查、真实任务
验证四种保证（"≥1000 条源码证据标记"统计的是固定产物里上游 bundle 的证据注释行）；把"先哈希、后写入"改为实际的
"先写 `.partial`、校验通过才改名"（保证是未通过校验的下载不会成为正式缓存文件）；把 `node --test` 的代价从"没有
覆盖率/快照/分片能力"改为"本项目尚未配置这些能力"（Node 自带相应参数）；把两处动机推测从结论降级为观察加推断
（远端盒的动机、渲染层依赖与不发布源码之间的因果）。
